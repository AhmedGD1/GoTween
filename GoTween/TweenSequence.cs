...
public partial class TweenSequence : RefCounted
{
    ...
    public void Start()
    {
        ...
        var timeline = GoTween.CreateNewTween();
        
        var sortedSteps = new List<SequenceStep>(steps);
        sortedSteps.Sort((a, b) => a.StartTime.CompareTo(b.StartTime));
        ...
        foreach (var step in sortedSteps)
        {
            ...
            if (builder is not TweenBuilder tweenBuilder)
            {
                GD.PushError($"TweenSequence: Builder is not a TweenBuilder: {builder.GetType().Name}");
                continue;
            }
            ...
            float waitTime = step.StartTime - currentTime;
            
            if (waitTime > 0f)
            {
                timeline.TweenInterval(waitTime);
